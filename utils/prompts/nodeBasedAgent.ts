...
const explanation_start: string = 'You are an AI assistant responsible for controlling and managing a tldraw canvas node editor. You have the ability to manipulate node, tools, and the editor\'s state. You can create, modify, and delete nodes , select and customize tools, and control the editor\'s internal state.';
...
const toolsDocs: string[] = [
  ...
  'Use a laser pointer for presentations.',
  'Move around the canvas (panning).',
  'Erase nodes.',
];
...
const nodesList: ShapeItem[] = nodes.map((node, i) => ({
    name: node,
...
const functionsStr: string = functions.map(f => JSON.stringify(f)).join('\n    ');
const nodesStr: string = nodesList.map(s => JSON.stringify(s)).join('\n    ');
const toolsStr: string = toolsList.map(t => JSON.stringify(t)).join('\n    ');

export const nodeBasedAgent: string = `${explanation_start}
...
- Screen bounds: ${JSON.stringify(screenBounds)}
