...
describe('Regular Expression Tests', () => {
  ...
  const dateTestData = {
    passing: ['2023-11-21'],
    failing: [
      { input: '2023/11/21', reason: 'Date format should be YYYY-MM-DD' }
    ]
  };
  ...
  const urlTestData = {
    ...
    failing: [
      { input: 'example', reason: 'URL should include http:// or https:// and a domain' }
    ]
  };
  ...
  const phoneNumberTestData = {
    ...
    failing: [
      { input: '123-45-678', reason: 'Phone number should match the specified formats' }
    ]
  };
  ...
  const emailTestData = {
    ...
    failing: [
      { input: 'test@example', reason: 'Email should have a valid domain and extension' }
    ]
  };
  ...
  const runTests = (regex: any, testData: any, description: string) => {
    describe(description, () => {
      testData.passing.forEach((input: any) => {
        it(`should pass for valid format: ${input}`, () => {
          expect(regex.test(input)).toBeTruthy();
        });
      });
      testData.failing.forEach(({ input, reason }: any) => {
