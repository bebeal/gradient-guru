// https://github.com/jamiebuilds/tailwindcss-animate/blob/main/index.js#L13
import plugin from 'tailwindcss/plugin';

function filterDefault(values: { [s: string]: any } | ArrayLike<any>) {
...
export const AnimationTailwindcssPlugin = plugin(
  ({ addUtilities, matchUtilities, theme }) => {
    ...
    matchUtilities({ 'anim-duration': (value) => ({ animationDuration: value }) }, { values: filterDefault(theme('animationDuration')) });

    matchUtilities({ 'anim-delay': (value) => ({ animationDelay: value }) }, { values: theme('animationDelay') });
