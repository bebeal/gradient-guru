...
import { Accordion, Button, IconSet, IconSets, Input } from '@/components';
...
const IconSetsPage = () => {
  ...
  const handlePageChange = (iconSetName: string, newPage: number) => {
    const totalIcons = Object.entries(IconSets[iconSetName].icons).length;
  ...
  const getFilteredIcons = (iconSetName: string, IconSet: IconSet) => {
    const searchQuery = localSearchQueries[iconSetName]?.toLowerCase() || '';

    if (!searchQuery) {
      return Object.entries(IconSet.icons);
    }

    return Object.entries(IconSet.icons).filter(([iconName]) =>
  ...
  return (
    <div className='p-8'>
      ...
      {filteredIconSets.map((iconSetName: string) => {
        const IconSet = IconSets[iconSetName];
        const page = currentPage[iconSetName] || 0;
