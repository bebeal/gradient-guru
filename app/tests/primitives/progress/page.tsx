...
const DownloadComponent = () => {
  ...
  return (
    <div className="w-full flex h-auto flex-col flex-nowrap text-xs">
      <button className="bg-gray-400 hover:bg-gray-500" onClick={() => downloader.mutate()}>Download {url}</button>
      <div>Progress: {downloadPercentage.toFixed(2)}%</div>
      <div className="w-full flex">Bytes Downloaded: {bytesDownloaded} / {total || '??'}</div>
