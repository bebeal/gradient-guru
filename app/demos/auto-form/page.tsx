    ...
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <p className={`mt-1 text-red-500 text-sm h-5 ${formState.errors.name ? 'visible' : 'invisible'}`}>
        ...
        <div>
          ...
          <p className={`mt-1 text-red-500 text-sm h-5 ${formState.errors.email ? 'visible' : 'invisible'}`}>
            {formState.errors.email?.message?.toString()}
          </p>
        </div>
        {/* <button type="submit" className="w-full py-2 px-4 bg-blue-600 hover:bg-blue-700 rounded text-white font-medium transition-colors">Submit</button> */}
      </form>
    </div>
  );
};
...
const TouchedFormResults = ({ formState }: UseFormReturn<FormInputs, any, undefined>) => {
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-auto">
      ...
      <pre className="whitespace-pre-wrap break-words overflow-x-auto">{JSON.stringify(touched, null, 2)}</pre>
    </div>
  );
};
