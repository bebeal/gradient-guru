...
// dummy use session hook for now
const useSession = () => {
  return {
    data: {
      account: {
...
export const useDrivePicker = ({ googleDriveApiKey }: DrivePickerProps) => {
  ...
  const createPicker = useCallback(() => {
    ...
    const preSearchedView = new pickerApi.View(pickerApi.ViewId.DOCS).setQuery('.tldr');
  ...
  const GoogleApiScript = () => {
    return <script id="google-drive-api" src="https://apis.google.com/js/api.js" onLoad={loadGoogleApi} />;
  };
