...
export const BackgroundCellAnimation = () => {
  return (
    <div className="relative h-[400px] w-full bg-slate-950 flex justify-center overflow-hidden">
      <BackgroundCellCore />
      <div className="relative z-50 mt-40 pointer-events-none select-none">
        <h1 className="text-base font-medium text-center bg-clip-text text-transparent bg-gradient-to-b from-neutral-100 to-neutral-400 pointer-events-none">
...
const BackgroundCellCore = () => {
  ...
  const size = 300;
  return (
    <div ref={ref} onMouseMove={handleMouseMove} className="w-full h-full absolute inset-0">
      ...
      {matrix.map((row, rowIdx) => (
        <div key={`matrix-row-${rowIdx}`} className="flex flex-col  relative z-20 border-b">
          {row.map((column, colIdx) => {
            ...
            return (
              <div key={`matrix-col-${colIdx}`} className={cn('bg-transparent border-l border-b border-neutral-600', cellClassName)} onClick={() => setClickedCell([rowIdx, colIdx])}>
                <motion.div
                  initial={{
                    opacity: 0,
                  }}
