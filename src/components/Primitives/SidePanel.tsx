...
export const SidePanel = forwardRef((props: SidePanelProps, _ref?: ForwardedRef<HTMLElement>) => {
  ...
  const changeTab = useCallback(
    (e: any, _tabValue: string, index: number) => {
  ...
  return (
    <>
      <Tabs.Root
        className={cn(`relative flex w-auto h-full`, `text-primary shadow-2 shadow-2xl`, `transition-all z-[200]`, className)}
        data-orientation="vertical"
        orientation="vertical"
        activationMode="manual"
        value={activeTabIndex !== undefined ? `${tabs[activeTabIndex]?.name}` : undefined}
        {...rest}
      >
        ...
        {handle && (
          <div
            ...
            className={cn(
              'z-[206] absolute top-1/2 -translate-y-1/2 -translate-x-1/2 left-full flex h-4 w-3 items-center justify-center rounded-sm border bg-primary hover:bg-secondary !cursor-pointer',
              resizing ? `border-accent` : `border-primary`,
              activeTabIndex !== undefined && `border-accent`,
              cursor,
            )}
          >
            <Icon set="Carbon" icon="Draggable" className="h-2.5 w-2.5" />
          </div>
        )}
      </Tabs.Root>
    </>
