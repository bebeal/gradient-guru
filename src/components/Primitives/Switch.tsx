...
export const Switch = forwardRef<any, SwitchProps>((props, ref) => {
  ...
  const style: any = {
    ...
    boxShadow:
      isPressed === undefined
        ...
        : isPressed
          ? `${thumbSize} 0 0 2px ${baseColor} inset, 0 0 0 2px ${baseColor} inset`
  ...
  return (
    <div className={cn('relative flex w-auto h-auto justify-center items-center gap-1', readOnly && 'cursor-default pointer-events-none')}>
      {offLabel && <div className={cn('text-[10px] text-muted break-words text-center w-12', !internalPressed && `text-primary`)}>{offLabel}</div>}
      ...
      {onLabel && <div className={cn('text-[10px] text-muted break-words text-center w-12', internalPressed && `text-primary`)}>{onLabel}</div>}
    </div>
  );
});
