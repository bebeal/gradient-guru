...
type GpuParts = (typeof GPU_PARTS)[number];
type VariantSet = {
  color?: string;
  className?: string;
};

type GpuVariantSet = 'unique' | 'realistic';
const GpuPartVariantSets: Record<GpuVariantSet, Record<GpuParts, VariantSet>> = {
  unique: {
  ...
  realistic: {
    ...
    'fan-1': {
      color: '#BDC3C7',
      className: cn('fill-[url(#fanGradient)] stroke-[url(#fanGradient)] [box-shadow:inset_-2px_-2px_4px_#8a949e,inset_2px_2px_4px_#d5dce3]'),
    }, // silver gradient
    'fan-2': {
      color: '#BDC3C7',
      className: cn('fill-[url(#fanGradient)] stroke-[url(#fanGradient)] [box-shadow:inset_-2px_-2px_4px_#8a949e,inset_2px_2px_4px_#d5dce3]'),
...
export const ThreeFanGpu = forwardRef((props: ThreeFanGpuProps, ref?: Ref<SVGSVGElement>) => {
  const { variant = 'realistic', spinFans = true, outline = false, spinAnimation = 'animate-spin-ccw', className = '' } = props;
  const internalRef = useRef<SVGSVGElement | undefined>(undefined);
  ...
  const BaseGpuIcon = useCallback(
    (props: BaseGpuIconProps) => {
      ...
      return (
        <svg ref={ref || internalRef} id="gpu" viewBox="0 0 85 35.86" {...gpuStyles} className={gpuClassName} {...rest}>
    ...
    [variant, getPartProperties, ref, SvgRealisticGradients],
  );

  const ThreeFanGpuComponent = useMemo(() => {
    return <BaseGpuIcon gpuVariant={variant} className={cn(className, outline && `[&>*]:stroke-black [&>*]:[stroke-alignment:outside] [&>*]:[stroke-width:0.5] w-full h-full`)} />;
  }, [variant, outline, className, BaseGpuIcon]);

  return ThreeFanGpuComponent;
});
ThreeFanGpu.displayName = 'ThreeFanGpu';
