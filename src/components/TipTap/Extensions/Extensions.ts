 ...
 *          PRO EXTENSIONS
 * =================================
 * PRO EXTENSIONS REQUIRE TIPTAP PRO REGISTRY: https://cloud.tiptap.dev/pro-extensions
 * - [InvisibleCharacters]: Adds decorators to show non-printable characters and help you increase accessibility.
 * - [Mathematics]: Allows you to write and visualize even complex mathematical formulas or equations in your editor. Please note that the current version is still in a very basic stage.
 * - [TableOfContent]: Lets you get a list of anchors from your document and passes on important information about each anchor (for example the depth, the content and a unique ID for each heading but also the active state and scroll states for each anchor). This can be used to render the table of content on your own.
 * - [UniqueID]: Adds unique IDs to all nodes. The extension keeps track of your nodes, even if you split them, merge them, undo/redo changes, crop content, paste content … It just works. (Also, you can configure which node types get an unique ID, and which not, and you can customize how those IDs are generated).
 */
const ProExtensions = {
...
const NodeExtensions = {
  ...
  TaskItem: TaskItem.configure({
    ...
    onReadOnlyChecked: (_node: Node, _checked: boolean): boolean => {
 ...
 * - [Snippets]: Drag n droppable snippets which can be created ad-hoc for frequently repeated content
 */
