    ...
    <div className="p-2 bg-white dark:bg-black rounded-lg">
      <form onSubmit={state.handleSubmit} className="flex items-center gap-2">
        <label className="flex items-center gap-2 p-2 rounded-lg bg-neutral-100 dark:bg-neutral-900 cursor-text">
          <Icon set="Lucide" icon="Link" className="flex-none text-black dark:text-white" />
          <input
            type="url"
        ...
        <Button variant="classic" type="submit" disabled={!state.isValidUrl}>
          Set Link
        </Button>
      </form>
      <div className="mt-3">
        <label className="flex items-center justify-start gap-2 text-sm font-semibold cursor-pointer select-none text-neutral-500 dark:text-neutral-400">
          Open in new tab
          <Checkbox checked={state.openInNewTab} onChange={state.setOpenInNewTab} />
        </label>
      </div>
    </div>
  )
