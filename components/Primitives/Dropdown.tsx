...
export const DropdownMenu = (props: DropdownMenuProps) => {
  ...
  const DropdownMenuItem = useCallback((item: any, idx: number) => {
...
export const DropdownGroup = (props: DropdownGroupProps) => {
  ...
  return (
    <DropdownMenuPrimitive.Group className={cn('flex flex-col gap-1', className)} {...rest}>
      {items.map((item: any, idx: number) => <DropdownItem key={`group-item-${idx}`} {...item} />)}
        ...
        return (
            ...
            >
              <span className="relative inset-0 flex items-center justify-center leading-0 w-3 h-3">
                ...
                <DropdownMenuPrimitive.ItemIndicator><IconSetCache.Carbon.DotMark className={cn("absolute inset-0 w-3 h-3 text-accent", radioRadius && RadiusClasses(radioRadius))} /></DropdownMenuPrimitive.ItemIndicator>
              </span>
              {item.children}
            </DropdownMenuPrimitive.RadioItem>
        );
...
export const DropdownCheckboxItem = (props: DropdownCheckboxItemProps) => {
  ...
  return (
    <DropdownMenuPrimitive.CheckboxItem
      ...
      className={cn('group relative flex items-center gap-1 w-full h-[25px] px-[10px] cursor-pointer',
                    className
                )}
    >
      <span className="relative inset-0 flex items-center justify-center leading-0 w-3 h-3">
        ...
        <DropdownMenuPrimitive.ItemIndicator><IconSetCache.Carbon.Checkmark className={cn("absolute inset-0 w-2.5 h-2.5 left-[0.0625rem] top-[0.0625rem] text-primary")} /></DropdownMenuPrimitive.ItemIndicator>
      </span>
...
export const DropdownItem = (props: DropdownItemProps) => {
  ...
  return (
    <DropdownMenuPrimitive.Item
      ...
      tabIndex={0}
      className={cn('relative flex items-center justify-between gap-1 h-[25px] w-full px-[10px] cursor-pointer font-bold pointer-events-auto', className)}
    >
    {icon}
    {children}
    </DropdownMenuPrimitive.Item>
...
export const DropdownSeparator = () => {
  return (
    <Separator orientation='horizontal' className={'pointer-events-none cursor-default'} />
  );
};
