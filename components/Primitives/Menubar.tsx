...
export const Menubar = (props: MenubarProps) => {
  ...
  return (
    <MenubarPrimitive.Root
      className={cn('relative flex w-auto h-auto overflow-hidden bg-primary', RadiusClasses(radius), className)}
      {...menubarProps}
    >
      {items.map((menuItem, idx) => {
      const cornerClassname = idx === 0 ? `rounded-l-md` : idx === items.length - 1 ? `rounded-r-md` : `rounded-none`;
      return (
          <MenubarMenu key={`menu-item-${idx}`} className={cornerClassname} {...menuItem} />
        );
      })}
    </MenubarPrimitive.Root>
  )
...
export const MenubarGroup = (props: MenubarGroupProps) => {
  ...
  return (
    <MenubarPrimitive.Group className={cn('flex flex-col gap-1', className)} {...rest}>
      {items.map((item: any, idx: number) => <MenubarItem key={`group-item-${idx}`} {...item} />)}
...
export const MenubarItem = (props: MenubarItemProps) => {
  ...
  return (
    <MenubarPrimitive.Item
      {...props}
      className={cn('group relative flex items-center justify-between gap-1 h-[25px] w-full px-[10px] cursor-pointer',
                    className
                )}
    >
      <>
