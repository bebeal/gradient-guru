    ...
    <div className="bg-white rounded-lg shadow text-black text-opacity-80 text-sm overflow-hidden p-1 relative">
      {items.length ? (
        items.map((item: any, index: number) => (
          <button className={`bg-transparent border border-transparent rounded-md block m-0 py-1 px-2 text-left w-full ${index === selectedIndex ? 'border-black' : ''}`} key={index} onClick={() => selectItem(index)}>
            {item}
          </button>
        ))
      ) : (
