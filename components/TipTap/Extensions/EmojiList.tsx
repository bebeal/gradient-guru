  ...
  return (
    <div className="flex flex-col gap-1 p-[0.2rem] relative rounded-[0.5rem] bg-primary text-primary text-opacity-80 overflow-hidden text-[0.9rem] shadow-[0_0_0_1px_rgba(0,0,0,0.05),_0px_10px_20px_rgba(0,0,0,0.1)]">
      {items.map((item, index) => (
        <button className={`flex m-0 w-full text-left bg-transparent rounded-[0.4rem] border border-transparent p-[0.2rem_0.4rem] ${index === selectedIndex ? 'border-black' : ''}`} key={index} onClick={() => selectItem(index)}>
          {item.fallbackImage ? <Image src={item.fallbackImage} alt={item.name} width={12} height={12} className="w-[1em] h-[1em] flex" /> : item.emoji}:{item.name}:
        </button>
      ))}
    </div>
  );
